<%@ page language="java" pageEncoding="UTF-8"%>
 ...
 <link rel="stylesheet" type="text/css" href="view/style/css/properties/item.css">
 	...
 	<form method="get" name="itemListForm1" action="properties.do" style="width: 500px;">
 			...
 			<input type="hidden" name="openerIndex" value="${param.openerIndex }"/><br/>
 		...
 		<c:choose>
		 	<c:when test="${!empty requestScope.list }">
		 		<c:forEach var="item" items="${requestScope.list }">
				 	<tr class="listover" onMouseOver="this.style.backgroundColor='#F6F6F6'" onMouseOut="this.style.backgroundColor='#FFFFFF'" onclick="send('${item.mea_class_no}', '${item.class_kor_name }', '${item.grade }','${item.id }');" >
				 		<td colspan="2">
 ...
 <script type="text/javascript" >
 	...
 	function send(mea_class_no, slaveName, grade, id){
 		try{
 			
 			//console.log("mea_class_no : "+mea_class_no+"  slaveName : "+slaveName+"  grade : "+grade+"  id : "+id);
 			...
 			//console.log("slaveCodeList : "+slaveCodeList);
 			...
 			var slaveIdList = $("input[name=slaveCategoryId]", window.opener.document);
 			var slaveIdNode =$(slaveIdList).get(openerIndex); 
 			$(slaveIdNode).val(id);

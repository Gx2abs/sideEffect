 		...
 		<c:choose>
		 	<c:when test="${!empty requestScope.list }">
		 		<c:forEach var="item" items="${requestScope.list }">
				 	<tr class="listover" onMouseOver="this.style.backgroundColor='#F6F6F6'" onMouseOut="this.style.backgroundColor='#FFFFFF'"onclick="send('${item.mea_class_no}', ${item.grade },'${item.class_kor_name }','${item.id }');" >
				 		<td colspan="2">
...
<div class="btnType3">
 ...
 <script type="text/javascript" >
 	...
 	function send(mea_class_no, grade, name, id){
 		try{
 			...
 			var masterIdNode = $("input[name=masterId]", window.opener.document);
 			$(masterIdNode).val(id);

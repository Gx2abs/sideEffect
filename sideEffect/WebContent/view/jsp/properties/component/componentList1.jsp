...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
 ...
 <link rel="stylesheet" type="text/css" href="view/style/css/properties/item.css">
	...
	long privilegeId = -1;
 	...
 	<form method="get" name="componentListForm1" action="properties.do">
 			...
 			<c:choose>
		 		<c:when test="${!empty componentLevel }">
		 			<c:forEach var="componentLevel"  items="${componentLevel}">
		 				<c:choose>
			 				<%-- <c:when test="${componentLevel.id eq 1}"> --%>
			 				<c:when test="${componentLevel.id eq 0}">
	 							<input name="componentLevel" type="radio" value="${componentLevel.id }" checked="checked">${componentLevel.propertyValue }
	 						</c:when>
				 			<c:otherwise>
	 							<input name="componentLevel" type="radio" value="${componentLevel.id }">${componentLevel.propertyValue }
	 						</c:otherwise>
 						</c:choose>
			 		</c:forEach>
		 		</c:when>
 	...
 	<c:choose>
 		<c:when test="${!empty componentLevel }">
 			<c:forEach var="componentLevel"  items="${componentLevel}" begin="1" end="6">
	 			<img src="view/style/images/btn_lev${componentLevel.id}.jpg" id="submit1" class="listover" alt="level${componentLevel.id}등록" onclick="create(${componentLevel.id});"/>
	 		</c:forEach>
 		</c:when>
 		<c:otherwise>
 			No level data
		 	...
		 	<c:when test="${!empty requestScope.list }">
		 		<c:forEach var="item" items="${requestScope.list }">
				 	<tr class="listover" onclick="read(${item.id});" >
				 		...
				 		<td colspan="2">	
				 			<c:choose>
								<c:when test="${!empty item.componentHistory }">
									<c:forEach var="componentHistory" items="${item.componentHistory }" varStatus="i" >
										<c:choose>
 ...
 <script type="text/javascript" >
 	function read(id){
 		try{
 			//alert(id);
 			location.href = "properties.do?action=readComponent&menu=4&articleId="+id;
 		}catch(Exception){
 	...
 	function create(level){
 		location.href="properties.do?action=createComponentPage&menu=4&level="+level;
 	}

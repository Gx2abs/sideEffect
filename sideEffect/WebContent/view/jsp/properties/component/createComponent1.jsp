 ...
 <script type="text/javascript">
	...
	long privilegeId = -1;
 	...
 	<form name="componentCreateForm1"  method="POST" action="properties.do">
 		...
 		<input type="hidden" name="action" value="createComponent"/>
 	...
 	<script>
 		...
 		function checkChildren(){
 			...
 			try{
 				...
 				for(var i=2; i<currentLevel; i++){
 	 				//alert("i:="+i);
 	 				var nodeNameBase = "parentItem";
 	 				var targetName = "select[name="+nodeNameBase+i+"] option:selected";
 	 				
 	 				var target = $(targetName);
 	 				
 	 				//var selectedValue =target.val(); 
 	 				var selectedValue =target.text();
 	 				//alert("selectedValue : = " + selectedValue);
 	 				/* if(selectedValue <= 0 || "undefined"==selectedValue){ */
 	 					//if(selectedValue <= 0){
 	 						if(selectedValue == ""){
 	 					alert("setting result to false");
 	 					result = false;
 	 				}
 		...
 		function getChildList2(id, level, step){
 			...
 			var method = "GET";
 			var url = "properties.do?action=getChildComponent&parentItem="+id+"&level="+level+"&step="+step;
 			var isAsync = true;
		...
		function getChildList3(id, level, step){
	 			var xhr = new XMLHttpRequest();
	 			var method = "GET";
	 			var url = "properties.do?action=getChildComponent&parentItem2="+id+"&level="+level+"&step="+step;
	 			var isAsync = true;
	 			xhr.open(method, url, isAsync);
	 			xhr.send();
	 			xhr.onreadystatechange = function(){
		...
		function getChildList4(id, level, step){
				var xhr = new XMLHttpRequest();
				var method = "GET";
				var url = "properties.do?action=getChildComponent&parentItem3="+id+"&level="+level+"&step="+step;
				var isAsync = true;
				xhr.open(method, url, isAsync);
				xhr.send();
				xhr.onreadystatechange = function(){
		...
		function getChildList5(id, level, step){
			var xhr = new XMLHttpRequest();
			var method = "GET";
			var url = "properties.do?action=getChildComponent&parentItem4="+id+"&level="+level+"&step="+step;
			var isAsync = true;
			xhr.open(method, url, isAsync);
			xhr.send();
			xhr.onreadystatechange = function(){

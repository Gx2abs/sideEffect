...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
 ...
 <link rel="stylesheet" type="text/css" href="view/style/css/properties/item.css">
	...
	long privilegeId = -1;
 	...
 	<form name="componentCreateForm1"  method="POST" action="properties.do">
	 	...
	 	<table class="bordered fitToParent view_table" >
			...
			<tbody>
				...
				<tr class="itemProperty">
					<th class="tdHead createTextarea" >사유</th>
				...
				<tr class="itemProperty">
					...
					<td class="tdBody"  colspan="3">
						...
						<tr class="itemProperty">
							<%-- <td>
								<fmt:formatDate pattern="yyyy-MM-dd" value="${componentHistory.lastModified }"/>
							</td> --%>
							<td>
								<fmt:formatDate pattern="yyyy-MM-dd" value="${componentHistory.activeFrom }"/>
							</td>
							<td class="tdBody">
							<c:choose>
								<c:when test="${!empty requestScope.historyType }">
							...
							<td id="history${componentHistory.id }"><img src="view/style/images/btn_tab03.jpg" class="hover1" alt="이력삭제" onclick="deleteHistory(${componentHistory.id});"/></td>
								</c:forEach>
							</c:when>
						</c:choose>
					</table>
	 	...
	 	<div class="btnType3">
	 		<img src="view/style/images/btn_ok.jpg" id="submit1" alt="확인" class="buttonised" onclick="createComponent();"/>
 	...
 	<script>
 		function createComponent(){
 			if(document.componentCreateForm1.fdaCode.value != '${article.fdaCode }'){
	 	 		if(document.componentCreateForm1.check.value==0){
	 	 			alert("MFDS Code 중복검사를 해주세요.");
	 	 			return;
 	 			}
 			}
 			
 			$("form[name=componentCreateForm1]").submit();			
 		}
		...
		function deleteHistory(historyId){
 			...
 			var method="POST";
 			var url="properties.do?action=deleteComponentHistory&articleId="+historyId;
 			var isAsync=true;
 ...
 function duplicate(){
		var frm = document.componentCreateForm1;
		var fdaCode = frm.fdaCode.value;
		
		var xmlHttpRequest = new XMLHttpRequest();
		
		 var method = "POST";
		 var url = "properties.do?action=fdaCodeDuplicate&fdaCode="+fdaCode+"&table=COMPONENT_CODE";
		 var async = true;
 ...
 function keypress(){
	 document.componentCreateForm1.check.value=0;
 }
 </script>
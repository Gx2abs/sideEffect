...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>	
	...
	long privilegeId = -1;
 	...
 	<c:choose>
 		<c:when test="${!empty meaClassNoLevel }">
 			<c:forEach var="meaClassNo"items="${meaClassNoLevel}" begin="1" end="3">
	 			<img src="view/style/images/btn_class${meaClassNo.id }.jpg" id="submit1" class="listover" alt="level${meaClassNo.id }등록" onclick="create(${meaClassNo.id});"/>
	 		</c:forEach>
 		</c:when>
 		<c:otherwise>
 			No level data
 ...
 <script type="text/javascript" >
 	function read(mea_class_no, grade){
 		try{
 			location.href = "properties.do?action=readItem&menu=1&mea_class_no="+mea_class_no+"&grade="+grade;
 		}catch(Exception){
 	...
 	function create(level){
 		location.href="properties.do?action=createItemPage&menu=1&level="+level;

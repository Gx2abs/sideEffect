	...
	long privilegeId = -1;
	 	...
	 	<table class="bordered fitToParent view_table" >
			...
			<tbody>
				<tr class="itemProperty">
					...
					<td colspan="3">${meaClassNoLevel.propertyValue }</td>
				</tr>
				<tr class="itemProperty">
					<th class="tdHead" colspan="2">코드경로</th>
					<td class="tdBody">
						<c:choose>
							<c:when test="${article.class_level > 2}">
								${parent.higher_class_no } >
							</c:when>
						</c:choose>
						<c:choose>
							<c:when test="${article.class_level > 1}">
								${article.higher_class_no} >
							</c:when>
						</c:choose>
						...
						<select name="itemCodeType">
							 <c:choose>
							 	<c:when test="${!empty requestScope.itemCodeTypes  }">
							 		<c:forEach var="ict" items="${requestScope.itemCodeTypes }" begin="1" end="3">
							 			<c:choose>
				...
				<tr class="itemProperty">
					...
					<td class="tdBody">
						<input type="hidden" name="grade" value="${article.grade }"/>
							${article.gradeObj.propertyValue }
					</td>
				</tr>
				<tr class="itemProperty">
					<th class="tdHead" colspan="2">품목한글명</th>
					<td class="tdBody" colspan="3"><input type="text" name="class_kor_name" value="${article.class_kor_name }" /></td>
				</tr>
				<tr class="itemProperty">
					<th class="tdHead" colspan="2">품목영문명</th>
					<td class="tdBody" colspan="3"><input type="text" name="class_eng_name" value="${article.class_eng_name}" /></td>
				</tr>
				...
				<tr class="itemProperty">
					...
					<td class="tdBody" colspan="3">
						...
						<tr class="itemProperty createTextarea">
							...
							<td id="history${history.id }"><img src="view/style/images/btn_tab03.jpg" class="hover1" alt="이력삭제" onclick="deleteHistory('${history.id}','<%=privilegeId %>');"/></td>
								</c:forEach>
							</c:when>
						</c:choose>
					</table>
 	...
 	<script>
 		...
 		function deleteHistory(historyId,privilegeId){
 			...
 			var method="POST";
 			var url="properties.do?action=deleteItemHistory&articleId="+historyId+"&privilegeId="+<%=privilegeId %>;
 			var isAsync=true;

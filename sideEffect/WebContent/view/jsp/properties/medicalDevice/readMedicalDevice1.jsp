...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
 ...
 <link rel="stylesheet" type="text/css" href="view/style/css/properties/item.css">
	...
	long privilegeId = -1;
 	...
 	<form name="medicalDeviceCreateForm1" enctype="multipart/form-data">
 		<input type="hidden" name="action" value="readMedicalDevice"/>
					...
					<td class="tdBody" colspan="3">
						...
						<tr class="itemProperty">
							<%-- <td>
								...
								<c:choose>
									<c:when test="${!empty article.medicalDeviceMalfunctionHistory }">
										<li><fmt:formatDate pattern="yyyy-MM-dd" value="${medicalDeviceMalfunctionHistory.lastModified }"/></li>
									</c:when>
								</c:choose>
								</ul>
							</td> --%>
							<td>
								...
								<c:choose>
									<c:when test="${!empty article.medicalDeviceMalfunctionHistory }">
										<li><fmt:formatDate pattern="yyyy-MM-dd" value="${medicalDeviceMalfunctionHistory.activeFrom }"/></li>
									</c:when>
								</c:choose>
								</ul>
							</td>
							...
							<td>
								...
								<c:choose>
									<c:when test="${!empty article.medicalDeviceMalfunctionHistory }">
										<li>${medicalDeviceMalfunctionHistory.manager }</li>
									</c:when>
									<c:otherwise>
									<li>
									No record
							...
							<td colspan="2">
								<c:choose>
									<c:when test="${!empty medicalDeviceMalfunctionHistory.historyDescription }">
										...
										<textarea readonly="readonly" name="historyDescription"/>${medicalDeviceMalfunctionHistory.historyDescription }</textarea>
									</c:when>
								</c:choose>
							</td>
						</tr>
				...
				<%-- <tr class="itemProperty">
					...
					<td class="tdBody" colspan="3">
						...
						<c:choose>
							<c:when test="${!empty article.medicalDeviceMalfunctionHistory }">
								<c:forEach var="medicalDeviceMalfunctionHistory" items="${article.medicalDeviceMalfunctionHistory }">
									<li>${medicalDeviceMalfunctionHistory.historyDescription }</li>
								</c:forEach>
							</c:when>
							<c:otherwise>
								<li>
	 	...
	 	<div class="btnType3">
	 	<%if(privilegeId>1){ %>
		 	<img src="view/style/images/btn_edit.jpg" name="update" alt="update" class="buttonised" onclick="updateMedicalDevice(${article.id});"/>
 ...
 <script type="text/javascript">
  	function updateMedicalDevice(articleId){
  		
  		location.href="properties.do?action=updateMedicalDevicePage&menu=3&articleId="+articleId;
  		
  	}
  	
function del(articleId){
  		if(confirm("삭제하시겠습니까?")){
  			location.href="properties.do?action=deleteMedicalDevice&menu=3&articleId="+articleId;	
  		}
  		
  	}
  	
  	function toList(page, searchKeyword, searchColumn){
  		location.href="properties.do?action=listMedicalDevice&page=&searchColumn=&searchKeyword=";
  	}
 </script>

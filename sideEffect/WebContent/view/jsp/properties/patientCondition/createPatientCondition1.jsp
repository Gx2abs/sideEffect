<%@ page language="java" pageEncoding="UTF-8"%>
 ...
 <script type="text/javascript">
 	...
 	function duplicate(){
 		var frm = document.patientConditionCreateForm1;
 		...
 		var xmlHttpRequest = new XMLHttpRequest();
 		 ...
 		 var method = "POST";
 		 var url = "properties.do?action=fdaCodeDuplicate&fdaCode="+fdaCode+"&table=PATIENT_CONDITION";
 		 var async = true;
	...
	long privilegeId = -1;
	 	...
	 	<table class="bordered fitToParent view_table" >
			...
			<tbody>
				...
				<tr class="itemProperty createTextarea">
					...
					<!-- <td class="tdBody"  colspan="3"><input type="text" name="fdaSourceDefinitionKor" size="70"/></td> -->
				...
				<tr class="itemProperty createTextarea">
					...
					<!-- <td class="tdBody"  colspan="3"><input type="text" name="fdaSourceDefinition" size="70"/></td> -->
				...
				<tr class="itemProperty createTextarea">
					...
					<!-- <td class="tdBody"  colspan="3"><input type="text" name="ncitDefinitionKor" size="70"/></td> -->
					<td class="tdBody"  colspan="3"><textarea name="ncitDefinitionKor"/></textarea></td>
				</tr>
				<tr class="itemProperty createTextarea">
					<th class="tdHead" colspan="2">NCIt Definition</th>
					<!-- <td class="tdBody"  colspan="3"><input type="text" name="ncitDefinition" size="70"/></td> -->
 	...
 	<script>
 		function create(){
 			if(document.patientConditionCreateForm1.check.value==0){
 		...
 		function getChildList2(id, level, step){
 			...
 			var method = "GET";
 			var url = "properties.do?action=getChildPatientCondition&parentItem="+id+"&level="+level+"&step="+step;
 			var isAsync = true;
		...
		function getChildList3(id, level, step){
	 			var xhr = new XMLHttpRequest();
	 			var method = "GET";
	 			var url = "properties.do?action=getChildPatientCondition&parentItem2="+id+"&level="+level+"&step="+step;
	 			var isAsync = true;
	 			xhr.open(method, url, isAsync);
	 			xhr.send();
	 			xhr.onreadystatechange = function(){
		...
		function getChildList4(id, level, step){
				var xhr = new XMLHttpRequest();
				var method = "GET";
				var url = "properties.do?action=getChildPatientCondition&parentItem3="+id+"&level="+level+"&step="+step;
				var isAsync = true;
				xhr.open(method, url, isAsync);
				xhr.send();
				xhr.onreadystatechange = function(){
					...
					if(xhr.readyState==4 && xhr.status==200){
						...
						//after dom generation
						//$("select[name=parentItem4]").bind("mouseup", function(){
		...
		function getChildList5(id, level, step){
			var xhr = new XMLHttpRequest();
			var method = "GET";
			var url = "properties.do?action=getChildPatientCondition&parentItem4="+id+"&level="+level+"&step="+step;
			var isAsync = true;
			xhr.open(method, url, isAsync);
			xhr.send();
			xhr.onreadystatechange = function(){

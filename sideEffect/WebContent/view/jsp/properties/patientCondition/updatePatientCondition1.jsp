...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
 ...
 <link rel="stylesheet" type="text/css" href="view/style/css/properties/item.css">
	...
	long privilegeId = -1;
 	...
 	<form name="patientConditionCreateForm1"  method="POST" action="properties.do">
 		<input type="hidden" name="action" value="updatePatientCondition"/>
				...
				<tr class="itemProperty">
					...
					<td class="tdBody"  colspan="3">
						...
						<tr class="itemProperty">
							...
							<td>
								<fmt:formatDate pattern="yyyy-MM-dd" value="${patientConditionHistory.activeFrom }"/>
							</td>
							<td class="tdBody">
							<c:choose>
								<c:when test="${!empty requestScope.historyType }">
							...
							<c:choose>
								<c:when test="${!empty article.patientConditionHistory }">
										${patientConditionHistory.manager }
								</c:when>
							</c:choose>
							</td>
							<td>
							...
							<td id="history${patientConditionHistory.id }"><img src="view/style/images/btn_tab03.jpg" class="hover1" alt="이력삭제" onclick="deleteHistory(${patientConditionHistory.id});"/></td>
								</c:forEach>
							</c:when>
						</c:choose>
					</table>
	 	...
	 	<div class="btnType3">
	 		<img src="view/style/images/btn_ok.jpg" id="submit1" alt="확인" class="buttonised" onclick="createPatientCondition();"/>
 	...
 	<script>
 		function createPatientCondition(){
 			...
 			if(document.patientConditionCreateForm1.fdaCode.value != '${article.fdaCode }'){
		...
		function deleteHistory(historyId){
 			...
 			var method="POST";
 			var url="properties.do?action=deletePatientConditionHistory&articleId="+historyId;
 			var isAsync=true;
 ...
 function keypress(){
	 document.patientConditionCreateForm1.check.value=0;
 }
 </script>

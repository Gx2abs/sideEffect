...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
 ...
 <link rel="stylesheet" type="text/css" href="view/style/css/properties/item.css">
	...
	long privilegeId = -1;
 	...
 	<form method="get" name="reportTypeListForm1" action="properties.do">
 		...
 		<div class="searchBar1">	
 			...
 			<option value="name">보고종류명</option>
 	...
 	<table class=" bbsListType">
 		<thead>
 			<tr style="height:36px; padding: 0;">
 				...
 				<th width="318" style="background:url('view/style/images/bar_bg.jpg') repeat-x;">보고종류 명</th>
 				<th width="1" style="vertical-align: middle; padding:0;"><img src="view/style/images/bar_division.jpg"  border="0"/></th>
		 		...
		 		<c:forEach var="item" items="${requestScope.list }">
				 	<tr class="listover" onclick="read(${item.id});" >
				 		...
				 		<td colspan="2">
				 			...
				 			<c:choose>
								<c:when test="${!empty item.reportTypeHistory }">
									<c:forEach var="reportTypeHistory" items="${item.reportTypeHistory }" varStatus="i" >
										<c:choose>
 ...
 <script type="text/javascript" >
 	function read(id){
 		try{
 			location.href = "properties.do?action=readReportType&menu=8&articleId="+id;
 		}catch(Exception){
 	...
 	function create(){
 		location.href="properties.do?action=createReportTypePage&menu=8";
	...
	$(document).ready(function(){
 	...
 	function doGetSearch(){
 		$("form[name=reportTypeListForm1]").submit();
 	}

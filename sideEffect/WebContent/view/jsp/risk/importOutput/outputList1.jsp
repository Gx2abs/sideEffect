 	...
 	<table class=" bbsListType">
 		<thead>
 			<tr style="height:36px; padding: 0;">
 				...
 				<th width="1" style="vertical-align: middle; padding:0;"><img src="view/style/images/bar_division.jpg"  border="0"/></th>
 				<th width="80" style="background:url('view/style/images/bar_bg.jpg') repeat-x;">생산량</th>
 				<th width="1" style="vertical-align: middle; padding:0;"><img src="view/style/images/bar_division.jpg"  border="0"/></th>
		 	...
		 	<c:when test="${!empty requestScope.list }">
		 		<c:forEach var="item" items="${requestScope.list }">
				 	<tr class="listover" onclick="read(${item.id});" >
				 		...
				 		<td colspan="2">
				 			<fmt:parseNumber value="${item.itemOutput }" />
				 		</td>
				 		<td colspan="2">
				 			<c:if test="${!empty item.outputYear && !empty item.outputMonth }">
				 				${item.outputYear }-${item.outputMonth }
				 			</c:if>
				 		</td>
				 	</tr>
				 </c:forEach>
 ...
 <script type="text/javascript" >
	...
	function initItemList1Jsp(){
 	...
 	function read(id){
 		try{
 			//alert(id);
 			location.href = "importoutput.do?action=readOutput&menu=12&articleId="+id;
 		}catch(Exception){

...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
 	...
 	<form name="importOutputCreateForm1" method="POST" action="importoutput.do">
	 	<table class="bordered fitToParent view_table" >
			...
			<tbody>
				...
				<tr class="itemProperty">
					<th class="tdHead" >생산량</th>
					<td class="tdBody" ><fmt:parseNumber value="${article.itemOutput }" /></td>
				</tr>
				<tr class="itemProperty">
				...
				<tr class="itemProperty">
					...
					<td class="tdBody" >
						<c:if test="${!empty article.outputYear && !empty article.outputMonth }">
							${article.outputYear }-${article.outputMonth }
						</c:if>
					</td>
				</tr>
				<tr class="itemProperty" >
					...
					<td class="tdBody" >
						...
						<tr class="itemProperty">
							<%-- <td>
								...
								<c:choose>
									<c:when test="${!empty article.itemOutputHistory }">
										<li><fmt:formatDate pattern="yyyy-MM-dd" value="${itemOutputHistory.lastModified }"/></li>
									</c:when>
								</c:choose>
								</ul>
							</td> --%>
							<td>
								...
								<c:choose>
									<c:when test="${!empty article.itemOutputHistory }">
										<li><fmt:formatDate pattern="yyyy-MM-dd" value="${itemOutputHistory.activeFrom }"/></li>
									</c:when>
								</c:choose>
								</ul>
							</td>
							...
							<td>
								...
								<c:choose>
									<c:when test="${!empty article.itemOutputHistory }">
										<li>${itemOutputHistory.manager }</li>
									</c:when>
									<c:otherwise>
									<li>
									No record
 ...
 <script type="text/javascript">
  	function updateImportOutput(articleId){
  		...
  		location.href="importoutput.do?action=updateOutputPage&menu=12&articleId="+articleId;
  		
  	}
  	
function del(articleId){
  		if(confirm("삭제하시겠습니까?")){
  			location.href="importoutput.do?action=deleteOutput&menu=12&articleId="+articleId+"&privilegeId="+<%=privilegeId%>;	
  		}
  		
  	}
  	
  	function toList(page, searchKeyword, searchColumn){
  		location.href="importoutput.do?action=listOutput&page=&searchColumn=&searchKeyword=";
  	}
 </script>

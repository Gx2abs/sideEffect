	...
	long privilegeId = -1;
	 	...
	 	<table class="bordered fitToParent view_table" >
			...
			<tbody>
				...
				<tr class="itemProperty">
					...
					<td class="tdBody" ><input type="text" name="itemImport" value="<fmt:parseNumber value='${article.itemImport }' />" /></td>
				</tr>
				<tr class="itemProperty">
				...
				<tr class="itemProperty">
					...
					<td class="tdBody" ><input type="text" name="importDate" id="importDate" value="${article.importYear }-${article.importMonth}" /></td>	
				</tr>
				<tr class="itemProperty">
				...
				<tr class="itemProperty">
					...
					<td class="tdBody">
						...
						<tr class="itemProperty">
							<%-- <td><fmt:formatDate pattern="yyyy-MM-dd" value="${itemImportHistory.lastModified }"/></td> --%>
							<td><fmt:formatDate pattern="yyyy-MM-dd" value="${itemImportHistory.activeFrom }"/></td>
							<td class="tdBody">
							<c:choose>
								<c:when test="${!empty requestScope.historyType }">
									<c:forEach var="hType" items="${historyType }">
							...
							<td id="history${itemImportHistory.id }"><img src="view/style/images/btn_tab03.jpg" class="hover1" alt="이력삭제" onclick="deleteHistory('${itemImportHistory.id}','<%=privilegeId %>');"/></td>
								</c:forEach>
							</c:when>
						</c:choose>
					</table>
 	...
 	<script>
 		...
 		function deleteHistory(articleId,privilegeId){
 			if(privilegeId>1){
 			var method = "get";
 			var url = "importoutput.do?action=deleteImportHistory&articleId="+articleId+"&privilegeId="+privilegeId;

...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
 ...
 <link rel="stylesheet" type="text/css" href="view/style/css/properties/item.css">
	...
	long privilegeId = -1;
 	...
 	<%@include file="/view/jsp/common/simplePageTitle1.jsp" %>
 		<form method="get" name="itemCategoryGroupListForm1" action="risk.category.do">
 		...
 		<div class="searchBar1">
 			...
 			</select><input type="text" name="searchKeyword" value="${param.searchKeyword }"><img src="view/style/images/bt_search.jpg" class="hover1" alt="검색" id="getSearchButton1" onclick="goSearch();">
 	...
 	<table class=" bbsListType">
 		<thead>
 			<tr style="height:36px; padding: 0;">
 				...
 				<th width="400" style="background:url('view/style/images/bar_bg.jpg') repeat-x;">그룹명</th>
 				<th width="1" style="vertical-align: middle; padding:0;"><img src="view/style/images/bar_division.jpg"  border="0"/></th>
 				<th width="90" style="background:url('view/style/images/bar_bg.jpg') repeat-x;">구분</th>
 				<th width="1" style="vertical-align: middle; padding:0;"><img src="view/style/images/bar_division.jpg"  border="0"/></th>
		 	...
		 	<c:when test="${!empty requestScope.list }">
		 		<c:forEach var="item" items="${requestScope.list }">
				 	<tr class="listover" onclick="read(${item.id});" >
				 		...
				 		<td colspan="2">
				 			${item.typeStatus.propertyValue }
				 		</td>
				 		<td colspan="2">
				 			<c:choose>
								<c:when test="${!empty item.itemCategoryGroupHistory }">
									<c:forEach var="itemCategoryGroupHistory" items="${item.itemCategoryGroupHistory }" varStatus="i" >
										<c:choose>
										<c:when test="${i.last == true}">
										<fmt:formatDate pattern="yyyy-MM-dd" value="${itemCategoryGroupHistory.lastModified }"/>
										</c:when> 
										</c:choose>
									 </c:forEach> 
								</c:when>
 ...
 <script type="text/javascript" >
 	function read(id){
 		try{
 			//alert(id);
 			location.href = "risk.category.do?action=readItemCategoryGroup&menu=11&articleId="+id;
 		}catch(Exception){
 	...
 	function create(){
 		location.href="risk.category.do?action=createItemCategoryGroupPage&menu=11";
 		
 	}
 	
 	function goSearch(){
 		//document.itemCategoryGroupListForm1.submit();
 		$("form[name=itemCategoryGroupListForm1]").submit();
 	}
 	
 	$( document ).ready(function() {
 	
 	    $('input[name="searchKeyword"]').keydown(function() {

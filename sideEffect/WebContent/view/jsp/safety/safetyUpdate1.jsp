...
/////////////////////////////////////////환자문제코드////////////////////////////////////////////////////////
		 ...
		 function codeDelete(report_id, code_id, gubun){
		 	//alert("report_id  "+report_id+"  code_id  "+code_id+"  gubun  "+gubun);
		 	 ...
		 	 var method = "POST";
		 	 var variable = "&report_id="+report_id+"&code_id="+code_id+"&gubun="+gubun+"&gubun2=2";
 	...
 	<div style="width: 970; margin: 20 20 20 20; overflow: hidden; ">
 	<img src="${titleImg }" />
		...
		<colgroup>
			...
			<tbody>
				...
				<tr>
					...
					<input type="text" class="datepicker" name="kfdaReportDate" id="kfdaReportDate" value="${fn:substring(article.kfdaReportDate,5,7)}/${fn:substring(article.kfdaReportDate,8,10)}/${fn:substring(article.kfdaReportDate,0,4)}" /></td>
				</tr>
				<tr>
 		...
 		<colgroup>
			...
			<tbody>
				<tr>
					...
					<td class="tdBody"><input type="text" value="${article.representative }" name="representatives" /></td>
				</tr>
				<tr>
					<th class="tdHead">담당자명</th>
					<td class="tdBody"><input type="text" value="${article.manager }" name="manager" /></td>
					<th class="tdHead"> 전화</th>
					<td class="tdBody"><input type="text" value="${article.telephone }" name="report_tel" /></td>
				</tr>
				<tr>
					...
					<tr>
						...
						<td colspan="3">
							<input type="text" value="${article.address }" size="70" name="report_address"/>
			...
			<tbody>
				<tr>
					...
					<td><input onclick="pop();" type="text" readonly value="${article.meb_item.company.entp_name}" class="buttonised" name="meb_item_manuf_name" />
						<input type="hidden" value="${article.meb_item.meddev_item_seq }" name="meb_item_manuf_id" />
						<input type="hidden" name="mebTypeInfoSelect" value="0" />
						<c:choose>
							<c:when test="${! empty article.safetyReportMebTypeInfo}">
					...
					<th class="tdHead">제조국/제조원</th>
						<td class="tdBody"><input type="text" value="${article.country_manufactured.propertyValue }" onclick="pop2();" readonly class="buttonised" name="countryManufactured" />
							<input type="hidden" name="country_manufactured_id" value="${article.country_manufactured_id }" />
							/
				...
				<tr>
					...
					<td class="tdBody" colspan="3">
						<input type="text" size="100" name="serial_number" value="${article.serial_number }" />
					</td>
				</tr>
				...
				<tr>
					...
					<th class="tdHead">제조원</th>
					<td class="tdBody"><input type="text" value="${article.meb_item.manuf_import_entp_name }" name="manuf_import_entp_name" /></td>
				</tr>
				<tr>
					<th rowspan="2">형명</th>
					<th class="tdHead">품목명</th>
					<td class="tdBody"><input type="text" value="${article.kfdaReportDate }" name="itemName2" /></td>
					<th class="tdHead"> 형명</th>
					<td class="tdBody"><input type="text" value="${article.meb_item.shape }" name="penalty" /></td>
				</tr>
				<tr>
					<th class="tdHead">제조번호</th>
					<td colspan="4"><input type="text" value="${article.id }" name="serialNumber"/></td>
				</tr> --%>
			</tbody>
 	</table>
 	...
 	<table class="bordered fitToParent view_table" >
		<colgroup>
			...
			<tr>
					...
					<td>
						<textarea name="safety_cause_content" style="width:100%; height:220;" id="safety_cause_content">${article.safety_cause_content }</textarea>
								<script type="text/javascript" >
			...
			<tr>
					...
					<c:choose>	
						...
						<input type="hidden" name="patient_code_null" value="1"  />	
							...
							<div id="divPatientCodeList_${patientCode.id }_1" width="100%">
				...
				<tr>
					...
					<td><%@include file="/view/jsp/safety/sideEffectReportType.jsp" %></td>
				<tr>
				<tr>
					<th class="tdHead" colspan="2">진행상황</th>
					<td><%@include file="/view/jsp/safety/reporterStatus.jsp" %></td>
				</tr>
					<tr>
			...
			<tbody>
				...
				<c:forEach var="file" items="${requestScope.fileList }" varStatus="i">
					<tr id="divFileList_${file.id}">
						...
						<td>
							<img src="view/style/images/btn_tab03.jpg" class="hover1" alt="삭제" onclick="fileDel1(${file.id}, ${article.id }, '${file.fullURL }')" />
						</td>
					</tr>
				</c:forEach>
 ...
 var divNum = 0;
	...
	function fileDel1(fileId, articleId, fullURL){
		//alert("파일삭제버튼  " + fileId+"  "+fullURL);
		divNum = fileId;
		var xmlHttpRequest = new XMLHttpRequest();
		
		var isAsynch = true;
		var requestMethod = "POST";
		var requestURL = "safetyReport.do?action=fileDelete&fileId="+fileId+"&articleId="+articleId+"&fullURL="+fullURL;
...
$(document).ready(function(){
 	...
 	function initPage(){
		...
		$("#arrReportDate_2").datepicker("option", "dateFormat", "yy-mm-dd");
	 	...
	 	$("#kfdaReportDate").datepicker();
		$("#kfdaReportDate").datepicker("option", "dateFormat", "yy-mm-dd");

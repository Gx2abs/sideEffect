...
<SCRIPT LANGUAGE=javascript FOR=XPlatformAXCtrl EVENT="loadingglobalvariables(strurl)">  
	XPlatformAXCtrl.CallScript("gfn_callscript('sideEffectPop::popPatC.xfdl')");      // 마이플랫폼의 InitUrl설정방식이 바뀜
</SCRIPT>
...
<SCRIPT LANGUAGE="javascript" FOR="XPlatformAXCtrl" EVENT="usernotify(nNotifyID,strMsg)">
	...
	//	for(var i=0; i<idArr.length; i++){
		...
		//	opener.document.safetyCreateForm1.patient_code_id.value = idArr[i];	
			opener.document.safetyCreateForm1.patient_code_id.value = strMsg;
	//	}
	...
	if(nNotifyID == 4){
		...
		opener.document.safetyCreateForm1.patient_code_user_view.value = view;		
	}
